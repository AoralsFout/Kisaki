/**
 * 翻译兜底
 *
 * 当模型未自行给出某个语言版本（say 缺字段，或干脆没调用 say 而直接输出正文）时，
 * 由系统确定性地补出译文，保证"双语永不破"。
 *
 * 基于 quickChat（非流式、低温），并要求保留原本语气；失败时回退原文，
 * 确保即便翻译不可用，显示/语音也不会变空。
 */
import { quickChat } from './client'
import { langName } from './langNames'
import { createLogger } from '../utils/logger'

const log = createLogger('Translate')
const TRANSLATION_CACHE_LIMIT = 100
const translationCache = new Map<string, string>()

function cacheTranslation(key: string, value: string) {
  translationCache.delete(key)
  translationCache.set(key, value)
  if (translationCache.size > TRANSLATION_CACHE_LIMIT) {
    translationCache.delete(translationCache.keys().next().value!)
  }
}

export interface TranslateOptions {
  /** 说话者人设，用于让译文更贴合角色语气（可选） */
  persona?: string
  /** 输出将直接送入 TTS，要求改写为仅含可朗读文字、空格和半角逗号的文本 */
  ttsSafe?: boolean
  /** 取消信号 */
  signal?: AbortSignal
  requestId?: string
  turn?: number
}

/**
 * 把文本翻译为目标语言，保留语气。
 *
 * @param text       原文
 * @param targetLang 目标语言代码（如 "ja-JP"）
 * @returns 译文；若原文为空或翻译失败，回退为原文 trim 结果
 */
export async function translateText(
  text: string,
  targetLang: string,
  opts?: TranslateOptions,
): Promise<string> {
  const trimmed = (text ?? '').trim()
  if (!trimmed) return ''
  const cacheKey = JSON.stringify([trimmed, targetLang, opts?.persona || '', Boolean(opts?.ttsSafe)])
  const sensitiveContext = {
    requestId: opts?.requestId,
    turn: opts?.turn,
    target_lang: targetLang,
    tts_safe: Boolean(opts?.ttsSafe),
  }
  const cached = translationCache.get(cacheKey)
  if (cached !== undefined) {
    // LRU：命中后移到末尾。
    translationCache.delete(cacheKey)
    translationCache.set(cacheKey, cached)
    log.debug('translate.cache_hit', '翻译缓存命中', { requestId: opts?.requestId, turn: opts?.turn, target_lang: targetLang, text_length: trimmed.length, tts_safe: Boolean(opts?.ttsSafe) })
    log.sensitiveDebug('translate.result_sensitive.debug', '翻译缓存命中文本', {
      ...sensitiveContext,
      source_text: trimmed,
      translated_text: cached,
      cached: true,
    })
    return cached
  }

  const targetName = langName(targetLang)
  const personaLine = opts?.persona ? `说话者的人设：${opts.persona}\n` : ''
  const outputRule = opts?.ttsSafe
    ? `输出将直接送入语音合成。即使原文已经是${targetName}，也要按以下规则改写：` +
      `只允许可朗读文字、语言正常所需的空格和半角逗号，半角逗号是唯一允许的标点和分句符号；` +
      `禁止其他标点、换行、Markdown、emoji、颜文字、特殊符号、阿拉伯数字、罗马数字和数学符号；` +
      `数字、百分比、日期、时间、版本号、金额、运算式、网址、路径、代码和缩写必须按语义写成${targetName}的可读口语。`
    : ''

  try {
    log.sensitiveDebug('translate.request_sensitive.debug', '翻译输入文本', {
      ...sensitiveContext,
      source_text: trimmed,
    })
    const out = await quickChat(
      [
        {
          role: 'system',
          content:
            `你是翻译助手。${personaLine}把用户消息翻译成${targetName}，` +
            `保留原本的语气、称呼与情感。${outputRule}` +
            `只输出译文或改写后的台词本身，不要任何解释、标注或引号。`,
        },
        { role: 'user', content: trimmed },
      ],
      opts?.signal,
      undefined,
      { requestId: opts?.requestId, turn: opts?.turn },
    )
    const result = out.trim()
    if (!result) {
      log.warn("translate.translate_text.warn", "翻译返回空，回退原文", undefined, { requestId: opts?.requestId, turn: opts?.turn })
      log.sensitiveDebug('translate.result_sensitive.debug', '翻译输出为空，回退原文', {
        ...sensitiveContext,
        source_text: trimmed,
        translated_text: trimmed,
        fallback: true,
      })
      return trimmed
    }
    cacheTranslation(cacheKey, result)
    log.debug("translate.translate_text.debug", `翻译完成 → ${targetLang} (${trimmed.length}→${result.length} 字)`, { requestId: opts?.requestId, turn: opts?.turn, target_lang: targetLang, trimmed_length: trimmed.length, result_length: result.length })
    log.sensitiveDebug('translate.result_sensitive.debug', '翻译输出文本', {
      ...sensitiveContext,
      source_text: trimmed,
      translated_text: result,
    })
    return result
  } catch (err) {
    log.warn("translate.translate_text.warn", `翻译失败，回退原文: ${(err as Error).message}`, err, { requestId: opts?.requestId, turn: opts?.turn })
    log.sensitiveDebug('translate.result_sensitive.debug', '翻译失败，回退原文', {
      ...sensitiveContext,
      source_text: trimmed,
      translated_text: trimmed,
      fallback: true,
    })
    return trimmed
  }
}
