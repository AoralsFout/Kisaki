...
export const webSearchTool: Tool = {
  ...
  handler: async (args) => {
    ...
    log.debug("tool_web_search.module.debug", `联网搜索 (provider=${config.provider}, count=${count}, time=${timeRange})`, { query_length: query.length, config_provider: config.provider, count, time_range: timeRange })
    log.sensitiveDebug("tool_web_search.query_sensitive.debug", "联网搜索词", { query })

    try {
      const hits = await search(config, query, count, timeRange)
      if (!hits.length) {
        log.info("tool_web_search.module.info", "搜索无结果", { query_length: query.length })
      ...
      log.info("tool_web_search.module.info", `搜索成功: ${hits.length} 条结果`, { query_length: query.length, hits_length: hits.length })
      return text.slice(0, RESULT_MAX)
    } catch (err) {
      const e = err as Error
      if (e.name === 'TimeoutError' || e.name === 'AbortError') {
        log.warn("tool_web_search.module.warn", "搜索超时", undefined, { query_length: query.length })
        return `搜索 "${query}" 超时，请稍后重试`
      }
      log.warn("tool_web_search.module.warn", "搜索失败", undefined, { query_length: query.length })
      log.sensitiveDebug("tool_web_search.error_sensitive.debug", "联网搜索错误详情", { message: e.message })
