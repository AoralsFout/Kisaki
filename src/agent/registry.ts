...
export function getDefinitions(context: CharacterToolContext): ToolDefinition[] {
  ...
  for (const t of tools.values()) {
    ...
    if (context.hasWorkspace === false && requiresWorkspace(toolName)) continue
    ...
    if (props) {
      ...
      // ② Live2D 枚举注入（来自 CharacterRuntime 的 renderer 能力快照）
      if (capabilities) {
        if (props.expression && capabilities.emotions.length) {
          props.expression.enum = [...capabilities.emotions]
          props.expression.description = `可选表情: ${capabilities.emotions.map(id => {
            const desc = capabilities.emotionDescriptions[id]
            return desc ? `${id}(${desc})` : id
          }).join('、')}`
        }
        if (props.motion && capabilities.motions.length) {
          props.motion.enum = capabilities.motions.map(motion => motion.group)
          props.motion.description = `可选动作: ${capabilities.motions.map(motion => (
            motion.description ? `${motion.group}(${motion.description})` : motion.group
          )).join('、')}`
        }
      }
    }
