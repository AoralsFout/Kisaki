...
import { POSE_PRESETS, useCharacterStore, loadCharacterJson, loadLive2DManifest } from '../../character'
...
// ── 调试角色（固定快照，不受 charStore 后续切换影响） ──
const devCharId = ref(charStore.currentId)
...
onMounted(async () => {
  ...
  // snapshot 当前角色 ID，独立加载数据
  devCharId.value = charStore.currentId
  try {
    devCharData.value = await loadCharacterJson(devCharId.value)
    if (devCharData.value?.render === 'live2d' && devCharData.value.live2d) {
      l2dManifest.value = await loadLive2DManifest(devCharId.value, devCharData.value)
    }
  } catch (e) {
    log.warn('调试角色加载失败: %s', (e as Error).message)
  }

  log.info('DevPanel 挂载 %s (角色: %s)', isDevWindow.value ? '(独立窗口)' : '(嵌入式)', devCharId.value)
