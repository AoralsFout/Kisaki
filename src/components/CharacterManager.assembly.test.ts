...
vi.mock('./CharacterAppearanceEditor.vue', () => ({ default: defineComponent({ props: ['manifestError', 'previewTarget'], emits: ['edit'], template: '<div class="appearance-editor-stub" :data-preview-target="previewTarget"><p v-if="manifestError" class="manifest-error">{{ manifestError }}</p><button class="appearance" @click="$emit(\'edit\', { type: \'add-pose\', value: \'happy\' })">appearance</button></div>' }) }))
...
describe('CharacterManager 装配', () => {
  ...
  it('按页面事件装配创建/外观/音色模块，并转发外观编辑意图', async () => {
    ...
    expect(wrapper.get('.appearance-editor-stub').attributes('data-preview-target')).toBe('#character-illustration-preview')
    expect(wrapper.get('#character-illustration-preview').element.parentElement?.classList).toContain('editor-view')
