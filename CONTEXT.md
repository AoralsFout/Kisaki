...
**日志 v2 契约**：
共享声明文件中的诊断日志条目规则，包含版本、字段形状、允许级别、必填字段、未知字段策略与事件名文法；前端和 Rust 适配器都从该声明读取约束。

**诊断日志条目** (`LogEntry`)：
符合日志 v2 契约的结构化 JSONL 记录，包含时间、级别、命名空间、文案、来源与事件名等字段。

**解析失败条目** (`logger.parse_failed`)：
读取或接收的非空输入不符合日志 v2 契约时，用于展示该输入解析失败的警告条目。
